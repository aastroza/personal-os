---
last_updated: YYYY-MM-DD
---

# {{PROJECT / STREAM NAME}} — {{work|personal|both}}

**Role:** {{your role}} · **Status:** {{green/yellow/red}} · **Horizon:** {{90-day outcome}}

## Why it matters
{{one-liner}}

## Current focus
{{the one move}}

## Current state
{{Last meaningful change, constraints, and what is true now. Keep tasks out of this section.}}

## Next milestone
{{Observable milestone + target date, if known}}

## Source of truth
- **Primary docs:** {{links or paths}}
- **Data / systems:** {{links or names}}
- **Related memory (optional; only when enabled):** {{explicit links to people, experiments, decisions, or results}}

## Developer context (optional)
- **Repository:** {{URL or local identifier}}
- **Default branch:** {{branch}}
- **Setup / run:** {{link or named repository entry point}}
- **Validation:** {{link or named repository entry point}}
- **CI / deploy:** {{workflow and environment links}}

> Code, issues, pull requests, CI logs, releases, technical documentation, and executable commands stay in the repository. Link them; do not copy them into PersonalOS or memory.

## TASKS
> Lives in TASKS.md next to this file (P0–P3).
